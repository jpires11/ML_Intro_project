...
def compare_predictions(file_list, save = False):
    plt.figure(figsize=(8, 6))
    # Read the first CSV file into a pandas DataFrame, skipping the first row
    for path in file_list:
        print(path)
        data = pd.read_csv(path, skiprows=1,names=['ID', 'RT'])
        y_pred = data.sort_values(by="RT")
        x_values = list(range(1, len(y_pred["ID"]) + 1))
    ...
    plt.legend()
    if (save == True):
        plt.savefig(os.path.join("visualisation", 'ComparePredictions.jpg'))
    plt.show()