...
def compare_predictions(file_path1, file_path2):
    ...
    y_pred_1 = data1.sort_values(by="RT_1")
    y_pred_2 = data2.sort_values(by="RT_2")

    x_values = list(range(1, len(y_pred_1["ID"]) + 1))
    ...
    plt.plot(x_values, y_pred_1["RT_1"], label=file_name1)
    plt.plot(x_values, y_pred_2["RT_2"], label=file_name2)
    plt.xlabel('ID')
